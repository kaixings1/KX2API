...
interface SettingsState {
  ...
  logCategoryConfigs: Record<string, { level: string; enabled: boolean }>
  setLogCategoryConfigs: (configs: Record<string, { level: string; enabled: boolean }>) => Promise<void>
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      logCategoryConfigs: {},
      setLogCategoryConfigs: async (configs) => {
        set({ logCategoryConfigs: configs })
        try {
          await window.electronAPI.logs.updateCategoryConfig(configs)
        } catch (error) {
          console.error('Failed to update log category config:', error)
        }
      },

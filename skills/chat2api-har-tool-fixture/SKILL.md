...
Use `--har` for the source HAR, `--client` for the fixture client profile, and `--out` for the output fixture path. If omitted, `--har` can come from `KX2API_HAR`, `--client` can come from `KX2API_CLIENT_PROFILE`, and output defaults to `backup/har/<client>-tool-fixtures-<timestamp>.json`.

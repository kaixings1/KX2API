...
test('management helper prints safe dry-run output without leaking secret', () => {
  const result = spawnSync('node', [script, 'snapshot', '--dry-run'], {
    env: {
      ...process.env,
      KX2API_BASE_URL: 'http://127.0.0.1:8080',
...
const baseUrl = (process.env.KX2API_BASE_URL || 'http://127.0.0.1:8080').replace(/\/$/, '')
const managementSecret = process.env.KX2API_MGMT_SECRET || ''
...
async function request(path, options = {}) {
  if (!managementSecret) throw new Error('KX2API_MGMT_SECRET is required')
...
```bash
KX2API_MGMT_SECRET=mgmt_xxx node skills/chat2api-management-api/scripts/management-api.mjs snapshot
KX2API_MGMT_SECRET=mgmt_xxx node skills/chat2api-management-api/scripts/management-api.mjs create-api-key --name codex-live-test
KX2API_MGMT_SECRET=mgmt_xxx node skills/chat2api-management-api/scripts/management-api.mjs delete-api-key --id key-id
```
...
const harPath = args.har || process.env.KX2API_HAR
const outPath = args.out || `backup/har/${args.client || 'custom-har'}-tool-fixtures-${Date.now()}.json`
const clientProfile = args.client || process.env.KX2API_CLIENT_PROFILE || 'custom-har'

if (!harPath) throw new Error('--har or KX2API_HAR is required')
...
test('replay dry-run reports selected fixture and model', () => {
  const result = spawnSync('node', [script, '--fixture', 'sample.json', '--profile', 'cherry-studio', '--model', 'deepseek-v4-flash', '--dry-run'], {
    env: {
      ...process.env,
      KX2API_API_KEY: 'sk-secret-value',
    },
    encoding: 'utf8',
  })
...
```json
{
  ...
  "toolsProtocol": {
    ...
    "rawWrapperPattern": "<\\\\|KX2API\\\\|tool_calls>|<tool_calls>|\\\\[function_calls\\\\]"
  },
  "promptProtocol": {
...
const args = parseArgs(process.argv.slice(2))
const baseUrl = (process.env.KX2API_BASE_URL || 'http://127.0.0.1:8080').replace(/\/$/, '')
const apiKey = process.env.KX2API_API_KEY || ''
...
async function main() {
  ...
  const model = args.model || process.env.KX2API_MODEL
  if (!args.fixture) throw new Error('--fixture is required')
  if (!model) throw new Error('--model or KX2API_MODEL is required')
  ...
  if (!apiKey) throw new Error('KX2API_API_KEY is required')
...
```bash
KX2API_MGMT_SECRET=mgmt_example \

...
const electronAPI = {
  ...
  tasks: {
    ...
    setSchedule: (id: string, scheduledAt: number | null, autoExecute: boolean): Promise<TaskRecord | null> =>
      ipcRenderer.invoke(IpcChannels.TASKS_SET_SCHEDULE, id, scheduledAt, autoExecute).then((r: any) => r.success ? (r.data as TaskRecord | null) : null),

    onAutoExecuted: (callback: (event: { taskId: string }) => void): (() => void) => {
      const handler = (_event: Electron.IpcRendererEvent, event: { taskId: string }) => callback(event)
      ipcRenderer.on(IpcChannels.TASKS_AUTO_EXECUTED, handler)
      return () => ipcRenderer.removeListener(IpcChannels.TASKS_AUTO_EXECUTED, handler)
    },
  },

...
import { stepfunSessionManager } from './stepfunSessionManager'
import { cookieSessionManager } from './cookieSessionManager'
import { kimiSessionManager } from './kimiSessionManager'
    ...
    return new Promise((resolve) => {
      ...
      const completeHandler = (result: { success: boolean; credentials?: Record<string, string>; error?: string }) => {
        ...
        if (result.success && result.credentials) {
          ...
          // Inject credentials into persistent partitions so next app launch
          // does not require re-login.
          if (providerType === 'stepfun') {
            stepfunSessionManager.injectCredentials(result.credentials).catch((e) => {
              console.error('[OAuthManager] Failed to inject StepFun credentials:', e)
            })
          } else {
            cookieSessionManager.injectCredentials(providerType, result.credentials).catch((e) => {
              console.error(`[OAuthManager] Failed to inject ${providerType} credentials:`, e)
            })
          }
      ...
      const validateAndComplete = async () => {
        ...
        try {
          ...
          if (providerType === 'minimax') {
          ...
          } else if (providerType === 'stepfun') {
            const oasisToken = collectedTokens['Oasis-Token']
            const webId = collectedTokens.device_id || collectedTokens.web_id || ''
            ...
            // Pass the token as 'token' and all collected data for validation
            validationCredentials = { token: oasisToken, ...collectedTokens }
            finalCredentials = { token: oasisToken, web_id: webId || '' }
            if (collectedTokens.cookies) {
              finalCredentials.cookies = collectedTokens.cookies
            }
            console.log('[OAuthManager] StepFun: Using Oasis-Token as token, web_id:', webId ? 'present' : 'missing', 'cookies:', Object.keys(collectedTokens).filter(k => k !== 'Oasis-Token' && k !== 'web_id'))
          } else {
          ...
          if (validation.valid) {
            const redacted = Object.keys(finalCredentials).reduce((r, k) => {
              const v = finalCredentials[k]
              r[k] = typeof v === 'string' && v.length > 8 ? v.slice(0, 6) + '...[REDACTED]' : v
              return r
            }, {} as Record<string, any>)
            console.log('[OAuthManager] Token is valid, completing login with credentials (redacted):', JSON.stringify(redacted, null, 2))

...
export class KimiSessionManager extends EventEmitter {
  ...
  /**
   * Inject JWT token into the persistent Kimi partition.
   *
  ...
  async injectCredentials(credentials: Record<string, string>): Promise<void> {
    if (!this.session) {
      console.warn('[KimiSession] Cannot inject credentials: session not initialized')
      return
    }

    try {
      const token = credentials.token || credentials.kimi_auth

      if (!token) {
        console.warn('[KimiSession] No token found in credentials, skipping injection')
        return
      }
      ...
      console.log('[KimiSession] Injected kimi-auth cookie (length:', token.length, ')')

      this.currentToken = token
      this.emit('token-found', token)
    } catch (error) {
      console.error('[KimiSession] injectCredentials failed:', error)
    }
  }

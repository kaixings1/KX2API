...
export class CookieSessionManager extends EventEmitter {
  ...
  /**
   * Inject credentials into a provider's persistent partition.
   *
   * Called by OAuthManager after a successful in-app login so the
   * persistent session can continue authenticated without requiring
   * the user to repeat the login flow on the next app launch.
   */
  async injectCredentials(providerType: ProviderType, credentials: Record<string, string>): Promise<void> {
    const partition = SESSION_PARTITION_PREFIX + providerType
    const config = getTokenExtractionConfig(providerType)
    if (!config) {
      console.warn(`[CookieSession] No extraction config for ${providerType}, skipping credential injection`)
      return
    }
    ...
    try {
      ...
      for (const source of config.tokenSources) {
        const key = source.alias || source.key
        ...
        console.log(`[CookieSession] Injected ${providerType} credential: ${source.key}`)
      }

      // Also inject any raw cookies returned by the login flow
      if (credentials.cookies && typeof credentials.cookies === 'object') {
        for (const [name, value] of Object.entries(credentials.cookies as Record<string, string>)) {
          if (!value) continue
          try {
            await ses.cookies.set({
              url: config.loginUrl,
            ...
            console.warn(`[CookieSession] Failed to inject cookie ${name} for ${providerType}:`, e)
          }
        }
      }
    } catch (error) {
      console.error(`[CookieSession] injectCredentials failed for ${providerType}:`, error)
    }
  }

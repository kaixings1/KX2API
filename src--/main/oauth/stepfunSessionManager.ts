...
export class StepFunSessionManager extends EventEmitter {
  ...
  /**
   * Inject credentials into the persistent StepFun partition.
   *
   * Called by OAuthManager after a successful in-app login so the
   * background BrowserWindow can continue from the authenticated state
   * instead of showing the login page again.
   */
  async injectCredentials(credentials: Record<string, string>): Promise<void> {
    if (!this.session) {
      console.warn('[StepFunSession] Cannot inject credentials: session not initialized')
      return
    }

    try {
      // Oasis-Token is the primary auth cookie.
      const token = credentials.Oasis_Token || credentials['Oasis-Token'] || credentials.token
      // Oasis-Webid binds the token to a device; fall back to the
      // deviceId/web_id credential when the cookie value isn't present.
      const webId = credentials.Oasis_Webid || credentials['Oasis-Webid'] || credentials.device_id || credentials.web_id
      ...
      if (token) {
        await this.session.cookies.set({
          ...
          name: 'Oasis-Token',
          value: token,
          domain: 'chat.stepfun.com',
          path: '/',
          httpOnly: true,
          secure: true,
        })
        console.log('[StepFunSession] Injected Oasis-Token cookie (length:', token.length, ')')
      }
      ...
      if (webId) {
        ...
        console.log('[StepFunSession] Injected Oasis-Webid cookie (length:', webId.length, ')')
      }

      // Preserve any other cookies returned by the login flow (e.g. affinity
      // cookies, session cookies). Keys that match known Oasis cookies are
      // skipped above; everything else lands here under its own name.
      const skipKeys = new Set(['Oasis_Token', 'Oasis-Token', 'Oasis_Webid', 'Oasis-Webid', 'device_id', 'web_id', 'token'])
      for (const [key, value] of Object.entries(credentials)) {
        if (skipKeys.has(key) || !value || key === 'cookies') continue
        try {
          await this.session.cookies.set({
            url: 'https://chat.stepfun.com/',
            name: key,
          ...
          console.log(`[StepFunSession] Injected extra cookie: ${key}`)
        } catch (e) {
          console.warn(`[StepFunSession] Failed to inject cookie ${key}:`, e)
        }
      }

      // Re-extract so listeners pick up the new values immediately.
      await this.extractCredentials()
    } catch (error) {
      console.error('[StepFunSession] injectCredentials failed:', error)
    }
  }

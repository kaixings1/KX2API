...
function generateToolDefinitions(tools: ChatCompletionTool[]): string {
  return tools
    .map((tool) => {
      const name = 'function' in tool ? tool.function.name : (tool as any).name
      const description = 'function' in tool ? tool.function.description : (tool as any).description
      const params = 'function' in tool && tool.function.parameters
        ? JSON.stringify(tool.function.parameters)
        : '{}'

      return `Tool \`${name}\`: ${description || 'No description'}. Arguments JSON schema: ${params}`
    })
    .join('\n')
}

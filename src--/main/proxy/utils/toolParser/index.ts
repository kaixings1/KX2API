...
import { managedXmlProtocol } from '../../toolCalling/protocols/managedXml'
...
export function parseToolCalls(content: string): ToolParseResult {
  ...
  switch (format) {
    ...
    case 'kimi':
      result = parseKimiFormat(content)
      break
    default:
      return { content, toolCalls: [], format: 'unknown', rawMatches: [] }
  }
...
/**
 * 解析 Kimi 格式工具调用
 * 格式: <|tool_calls_section_begin|><|tool_call_begin|> functions.Bash:1 <|tool_call_argument_begin|> {"command":"..."} <|tool_call_end|><|tool_calls_section_end|>
 */
function parseKimiFormat(content: string): { content: string; toolCalls: ToolCall[]; rawMatches: string[] } {
  const toolCalls: ToolCall[] = []
  const rawMatches: string[] = []
  let cleanContent = content

  // Match the entire tool calls section
  ...
  while ((sectionMatch = sectionPattern.exec(content)) !== null) {
    ...
    rawMatches.push(sectionMatch[0])

    // Match individual tool calls within the section
    // Format: <|tool_call_begin|> functions.Bash:1 <|tool_call_argument_begin|> {"command":"..."} <|tool_call_end|>
  ...
  // Remove matched sections from content
  for (const raw of rawMatches) {
    cleanContent = cleanContent.replace(raw, '')
  }

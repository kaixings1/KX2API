...
export async function registerIpcHandlers(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  // ==================== Plans Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.PLANS_GET_ALL, async (): Promise<PlanRecord[]> => {
    const { getAllPlans } = await import('../plans/plansService')
    return getAllPlans()
  })

  ipcMain.handle(IpcChannels.PLANS_GET_BY_ID, async (_, id: string): Promise<{ success: boolean; plan: PlanRecord | null }> => {
    const { getPlanById } = await import('../plans/plansService')
    const plan = getPlanById(id)
    return { success: !!plan, plan }
  })

  ipcMain.handle(IpcChannels.PLANS_CREATE, async (_, data: { title: string; description: string; steps: Array<{ id: string; description: string; status: string }> }): Promise<PlanRecord> => {
    const { createPlan } = await import('../plans/plansService')
    return createPlan(data)
  })

  ipcMain.handle(IpcChannels.PLANS_UPDATE, async (_, id: string, updates: Partial<PlanRecord>): Promise<PlanRecord | null> => {
    const { updatePlan } = await import('../plans/plansService')
    return updatePlan(id, updates)
  })

  ipcMain.handle(IpcChannels.PLANS_DELETE, async (_, id: string): Promise<boolean> => {
    const { deletePlan } = await import('../plans/plansService')
    return deletePlan(id)
  })

  ipcMain.handle(IpcChannels.PLANS_EXECUTE, async (_, id: string): Promise<{ success: boolean; error?: string }> => {
    const { executePlan } = await import('../plans/plansService')
    return executePlan(id)
  })

  // ==================== Tasks Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.TASKS_GET_ALL, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getAllTasks } = await import('../tasks/tasksService')
    return { success: true, data: getAllTasks() }
  })

  ipcMain.handle(IpcChannels.TASKS_GET_BY_ID, async (_, id: string): Promise<{ success: boolean; data: any | null }> => {
    const { getTaskById } = await import('../tasks/tasksService')
    const task = getTaskById(id)
    return { success: !!task, data: task }
  })

  ipcMain.handle(IpcChannels.TASKS_CREATE, async (_, data: any): Promise<{ success: boolean; data: any }> => {
    const { createTask } = await import('../tasks/tasksService')
    return { success: true, data: createTask(data) }
  })

  ipcMain.handle(IpcChannels.TASKS_UPDATE, async (_, id: string, updates: any): Promise<{ success: boolean; data: any | null }> => {
    const { updateTask } = await import('../tasks/tasksService')
    const task = updateTask(id, updates)
    return { success: !!task, data: task }
  })

  ipcMain.handle(IpcChannels.TASKS_DELETE, async (_, id: string): Promise<{ success: boolean }> => {
    const { deleteTask } = await import('../tasks/tasksService')
    return { success: deleteTask(id) }
  })

  ipcMain.handle(IpcChannels.TASKS_SET_STATUS, async (_, id: string, status: string): Promise<{ success: boolean; data: any | null }> => {
    const { setTaskStatus } = await import('../tasks/tasksService')
    const task = setTaskStatus(id, status)
    return { success: !!task, data: task }
  })

  ipcMain.handle(IpcChannels.TASKS_SET_SCHEDULE, async (_, id: string, scheduledAt: number | null, autoExecute: boolean): Promise<{ success: boolean; data: any | null }> => {
    const { updateTask } = await import('../tasks/tasksService')
    const task = updateTask(id, {
      scheduledAt,
      autoExecute,
      scheduledAtExecuted: false,
    })
    return { success: !!task, data: task }
  })
  ...
  ipcMain.handle(IpcChannels.GIT_GET_STATUS, async (_, repoPath: string): Promise<{ success: boolean; data: any | null }> => {
    try {
      gitService.setRepoPath(repoPath)
  ...
  ipcMain.handle(IpcChannels.GIT_CLONE, async (_, url: string, targetPath: string): Promise<{ success: boolean; error?: string }> => {
    try {
      gitService.clone(url, targetPath)
      return { success: true }
    } catch (e) {
      return { success: false, error: (e as Error).message }
    }
  ...
  ipcMain.handle(IpcChannels.GIT_PULL, async (_, repoPath: string): Promise<{ success: boolean; data?: { output: string } }> => {
    try {
      gitService.setRepoPath(repoPath)
      const output = gitService.pull()
      return { success: true, data: { output } }
  ...
  ipcMain.handle(IpcChannels.GIT_PUSH, async (_, repoPath: string): Promise<{ success: boolean; data?: { output: string } }> => {
    try {
      gitService.setRepoPath(repoPath)
      const output = gitService.push()
      return { success: true, data: { output } }
  ...
  ipcMain.handle(IpcChannels.GIT_GET_LOG, async (_, repoPath: string, limit = 20): Promise<{ success: boolean; data: any[] }> => {
    try {
      gitService.setRepoPath(repoPath)
      const log = gitService.getLog({ limit })
  ...
  ipcMain.handle(IpcChannels.GIT_GET_BRANCHES, async (_, repoPath: string): Promise<{ success: boolean; data: any[] }> => {
    try {
      gitService.setRepoPath(repoPath)
  ...
  ipcMain.handle(IpcChannels.GIT_CHECKOUT, async (_, repoPath: string, branchName: string): Promise<{ success: boolean; error?: string }> => {
    try {
      gitService.setRepoPath(repoPath)
  ...
  // ==================== Agents Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.AGENTS_GET_ALL, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getAllAgents } = await import('../agents/agentsService')
    return { success: true, data: getAllAgents() }
  })

  ipcMain.handle(IpcChannels.AGENTS_GET_BY_ID, async (_, id: string): Promise<{ success: boolean; data: any | null }> => {
    const { getAgentById } = await import('../agents/agentsService')
    const agent = getAgentById(id)
    return { success: !!agent, data: agent }
  })

  ipcMain.handle(IpcChannels.AGENTS_CREATE, async (_, data: any): Promise<{ success: boolean; data: any }> => {
    const { createAgent } = await import('../agents/agentsService')
    return { success: true, data: createAgent(data) }
  })

  ipcMain.handle(IpcChannels.AGENTS_UPDATE, async (_, id: string, updates: any): Promise<{ success: boolean; data: any | null }> => {
    const { updateAgent } = await import('../agents/agentsService')
    const agent = updateAgent(id, updates)
    return { success: !!agent, data: agent }
  })

  ipcMain.handle(IpcChannels.AGENTS_DELETE, async (_, id: string): Promise<{ success: boolean }> => {
    const { deleteAgent } = await import('../agents/agentsService')
    return { success: deleteAgent(id) }
  })

  ipcMain.handle(IpcChannels.AGENTS_EXECUTE, async (_, id: string, _input?: string): Promise<{ success: boolean; error?: string }> => {
    const { getAgentById } = await import('../agents/agentsService')
    const agent = getAgentById(id)
    if (!agent) return { success: false, error: 'Agent not found' }
    return { success: true }
  ...
  // ==================== Commands Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.COMMANDS_GET_ALL, async (): Promise<{ success: boolean; data: any[] }> => {
  ...
  ipcMain.handle(IpcChannels.COMMANDS_GET_BUILTIN, async (): Promise<{ success: boolean; data: any[] }> => {
    const all = commandRegistry.getAll()
    const data = all.map(c => ({
      id: c.name,
      name: c.name,
  ...
  ipcMain.handle(IpcChannels.COMMANDS_GET_CUSTOM, async (): Promise<{ success: boolean; data: any[] }> => {
    return { success: true, data: [] }
  })

  ipcMain.handle(IpcChannels.COMMANDS_ADD, async (_, command: any): Promise<{ success: boolean; data: any }> => {
    return { success: true, data: { ...command, id: command.name || `cmd_${Date.now()}` } }
  })

  ipcMain.handle(IpcChannels.COMMANDS_UPDATE, async (_, id: string, updates: any): Promise<{ success: boolean; data: any | null }> => {
    return { success: true, data: { id, ...updates } }
  })

  ipcMain.handle(IpcChannels.COMMANDS_DELETE, async (_, id: string): Promise<{ success: boolean }> => {
    return { success: true }
  })

  ipcMain.handle(IpcChannels.COMMANDS_EXECUTE, async (_event, name: string, _args?: string[]): Promise<{ success: boolean; data: any }> => {
    const cmd = commandRegistry.get(name)
    if (!cmd) {
      return { success: false, data: { success: false, error: `Command "${name}" not found`, durationMs: 0 } }
    }
    try {
      const startTime = Date.now()
      const result = await cmd.execute(_args || [])
      ...
      return {
        success: false,
        data: { success: false, error: (e as Error).message, durationMs: 0 },
      }
    }
  })

  // ==================== Workflows Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.WORKFLOWS_GET_ALL, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getAllWorkflows } = await import('../workflows/workflowsService')
    return { success: true, data: getAllWorkflows() }
  })

  ipcMain.handle(IpcChannels.WORKFLOWS_GET_BY_ID, async (_, id: string): Promise<{ success: boolean; data: any | null }> => {
    const { getWorkflowById } = await import('../workflows/workflowsService')
    const wf = getWorkflowById(id)
    return { success: !!wf, data: wf }
  })

  ipcMain.handle(IpcChannels.WORKFLOWS_CREATE, async (_, data: any): Promise<{ success: boolean; data: any }> => {
    const { createWorkflow } = await import('../workflows/workflowsService')
    return { success: true, data: createWorkflow(data) }
  })

  ipcMain.handle(IpcChannels.WORKFLOWS_UPDATE, async (_, id: string, updates: any): Promise<{ success: boolean; data: any | null }> => {
    const { updateWorkflow } = await import('../workflows/workflowsService')
    const wf = updateWorkflow(id, updates)
    return { success: !!wf, data: wf }
  })

  ipcMain.handle(IpcChannels.WORKFLOWS_DELETE, async (_, id: string): Promise<{ success: boolean }> => {
    const { deleteWorkflow } = await import('../workflows/workflowsService')
    return { success: deleteWorkflow(id) }
  })

  ipcMain.handle(IpcChannels.WORKFLOWS_EXECUTE, async (_, id: string, _input?: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { executeWorkflow } = await import('../workflows/workflowsService')
    const result = executeWorkflow(id, _input)
    if (result.success) {
  ...
  ipcMain.handle(IpcChannels.MCP_GET_CONFIG, async (): Promise<{ success: boolean; data: { servers: any[] } }> => {
    const { getServers } = mcpService
    return { success: true, data: { servers: getServers() } }
  })

  ipcMain.handle(IpcChannels.MCP_UPDATE_CONFIG, async (_, config: { servers: any[] }): Promise<{ success: boolean }> => {
    try {
      const existing = mcpService.getServers()
      const currentConfig = storeManager.getConfig() as Record<string, unknown>
      const updatedServers = [...existing, ...config.servers.filter(s => !existing.some(e => e.id === s.id))]
      storeManager.updateConfig({ mcp: { ...currentConfig.mcp, servers: updatedServers } } as Record<string, unknown>)
      return { success: true }
    } catch (e) {
  ...
  ipcMain.handle(IpcChannels.MCP_GET_SERVERS, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getServers } = mcpService
    return { success: true, data: getServers() }
  })

  ipcMain.handle(IpcChannels.MCP_GET_SERVER_BY_ID, async (_, id: string): Promise<{ success: boolean; data: any | null }> => {
    const { getServers } = mcpService
    const server = getServers().find(s => s.id === id) || null
    return { success: !!server, data: server }
  })

  ipcMain.handle(IpcChannels.MCP_ADD_SERVER, async (_, server: any): Promise<{ success: boolean; data: any }> => {
    const { addServer } = mcpService
    return { success: true, data: addServer(server) }
  })

  ipcMain.handle(IpcChannels.MCP_REMOVE_SERVER, async (_, id: string): Promise<{ success: boolean }> => {
    const { removeServer } = mcpService
    removeServer(id)
    return { success: true }
  })

  ipcMain.handle(IpcChannels.MCP_TEST_CONNECTION, async (_event, server: any): Promise<{ success: boolean; connected: boolean; tools: any[] }> => {
    return { success: true, connected: false, tools: [] }
  })

  ipcMain.handle(IpcChannels.MCP_GET_TOOLS, async (_event, serverId: string): Promise<{ success: boolean; data: any[] }> => {
    return { success: true, data: [] }
  })

  // ==================== Plugins Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.PLUGINS_GET_ALL, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getAllPlugins } = await import('../plugins/pluginsService')
    return { success: true, data: getAllPlugins() }
  })

  ipcMain.handle(IpcChannels.PLUGINS_GET_BUILTIN, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getBuiltinPlugins } = await import('../plugins/pluginsService')
    return { success: true, data: getBuiltinPlugins() }
  })

  ipcMain.handle(IpcChannels.PLUGINS_GET_INSTALLED, async (): Promise<{ success: boolean; data: any[] }> => {
    const { getInstalledPlugins } = await import('../plugins/pluginsService')
    return { success: true, data: getInstalledPlugins() }
  })

  ipcMain.handle(IpcChannels.PLUGINS_INSTALL, async (_, pluginId: string): Promise<{ success: boolean; error?: string }> => {
    const { installPlugin } = await import('../plugins/pluginsService')
    const result = installPlugin(pluginId)
    return result
  })

  ipcMain.handle(IpcChannels.PLUGINS_UNINSTALL, async (_, pluginId: string): Promise<{ success: boolean; error?: string }> => {
    const { uninstallPlugin } = await import('../plugins/pluginsService')
    const result = uninstallPlugin(pluginId)
    return result
  })

  ipcMain.handle(IpcChannels.PLUGINS_ENABLE, async (_, pluginId: string): Promise<{ success: boolean }> => {
    const { enablePlugin } = await import('../plugins/pluginsService')
    return enablePlugin(pluginId)
  })

  ipcMain.handle(IpcChannels.PLUGINS_DISABLE, async (_, pluginId: string): Promise<{ success: boolean }> => {
    const { disablePlugin } = await import('../plugins/pluginsService')
    return disablePlugin(pluginId)
  })

  ipcMain.handle(IpcChannels.PLUGINS_UPDATE, async (_, pluginId: string): Promise<{ success: boolean; error?: string }> => {
    const { updatePlugin } = await import('../plugins/pluginsService')
    const result = updatePlugin(pluginId)
    return result
  })

  ipcMain.handle(IpcChannels.PLUGINS_GET_BY_ID, async (_, id: string): Promise<{ success: boolean; data: any | null }> => {
    const { getPluginById } = await import('../plugins/pluginsService')
    const plugin = getPluginById(id)
    return { success: !!plugin, data: plugin }
  })

  // ==================== Other Config IPC Handlers ====================

  ipcMain.handle(IpcChannels.OTHER_CONFIG_GET, async (): Promise<{ success: boolean; data: any }> => {
    const { getOtherConfig } = await import('../otherConfig/otherConfigService')
    return { success: true, data: getOtherConfig() }
  })

  ipcMain.handle(IpcChannels.OTHER_CONFIG_UPDATE, async (_, updates: any): Promise<{ success: boolean }> => {
    const { updateOtherConfig } = await import('../otherConfig/otherConfigService')
    updateOtherConfig(updates)
    return { success: true }
  })

  ipcMain.handle(IpcChannels.OTHER_CONFIG_GET_ADVANCED, async (): Promise<{ success: boolean; data: Record<string, unknown> }> => {
    const { getAdvancedConfig } = await import('../otherConfig/otherConfigService')
    return { success: true, data: getAdvancedConfig() }
  })

  ipcMain.handle(IpcChannels.OTHER_CONFIG_UPDATE_ADVANCED, async (_, config: Record<string, unknown>): Promise<{ success: boolean }> => {
    const { updateAdvancedConfig } = await import('../otherConfig/otherConfigService')
    updateAdvancedConfig(config)
    return { success: true }
  })

  ipcMain.handle(IpcChannels.OTHER_CONFIG_RESET, async (): Promise<{ success: boolean }> => {
    const { resetOtherConfig } = await import('../otherConfig/otherConfigService')
    return resetOtherConfig()
  })

  // ==================== Tools Management IPC Handlers ====================

  ipcMain.handle(IpcChannels.TOOLS_GET_ALL, async (): Promise<{ success: boolean; data: any }> => {
    const { getAllToolsData } = await import('../tools/toolsService')
    return { success: true, data: getAllToolsData() }
  })

  ipcMain.handle(IpcChannels.TOOLS_GET_GROUPS, async (): Promise<{ success: boolean; data?: any[]; error?: string }> => {
    const { getAllToolsData } = await import('../tools/toolsService')
    return { success: true, data: getAllToolsData().groups }
  })

  ipcMain.handle(IpcChannels.TOOLS_GET_HINT_RULES, async (): Promise<{ success: boolean; data?: any[]; error?: string }> => {
    const { getAllToolsData } = await import('../tools/toolsService')
    return { success: true, data: getAllToolsData().hintRules }
  })

  ipcMain.handle(IpcChannels.TOOLS_ADD, async (_, tool: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { addTool } = await import('../tools/toolsService')
    return { success: true, data: addTool(tool) }
  })

  ipcMain.handle(IpcChannels.TOOLS_UPDATE, async (_, id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { updateTool } = await import('../tools/toolsService')
    const tool = updateTool(id, updates)
    return { success: !!tool, data: tool }
  })

  ipcMain.handle(IpcChannels.TOOLS_REMOVE, async (_, id: string): Promise<{ success: boolean; error?: string }> => {
    const { removeTool } = await import('../tools/toolsService')
    return { success: removeTool(id) }
  })

  ipcMain.handle(IpcChannels.TOOLS_TOGGLE, async (_, id: string): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { toggleTool } = await import('../tools/toolsService')
    const tool = toggleTool(id)
    return { success: !!tool, data: tool }
  })

  ipcMain.handle(IpcChannels.TOOLS_ADD_GROUP, async (_, group: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { addGroup } = await import('../tools/toolsService')
    return { success: true, data: addGroup(group) }
  })

  ipcMain.handle(IpcChannels.TOOLS_UPDATE_GROUP, async (_, id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { updateGroup } = await import('../tools/toolsService')
    const group = updateGroup(id, updates)
    return { success: !!group, data: group }
  })

  ipcMain.handle(IpcChannels.TOOLS_REMOVE_GROUP, async (_, id: string): Promise<{ success: boolean; error?: string }> => {
    const { removeGroup } = await import('../tools/toolsService')
    return { success: removeGroup(id) }
  })

  ipcMain.handle(IpcChannels.TOOLS_ADD_TO_GROUP, async (_, toolId: string, groupId: string): Promise<{ success: boolean; error?: string }> => {
    const { addToGroup } = await import('../tools/toolsService')
    return { success: addToGroup(toolId, groupId) }
  })

  ipcMain.handle(IpcChannels.TOOLS_REMOVE_FROM_GROUP, async (_, toolId: string, groupId: string): Promise<{ success: boolean; error?: string }> => {
    const { removeFromGroup } = await import('../tools/toolsService')
    return { success: removeFromGroup(toolId, groupId) }
  })

  ipcMain.handle(IpcChannels.TOOLS_ADD_HINT_RULE, async (_, rule: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { addHintRule } = await import('../tools/toolsService')
    return { success: true, data: addHintRule(rule) }
  })

  ipcMain.handle(IpcChannels.TOOLS_UPDATE_HINT_RULE, async (_, id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { updateHintRule } = await import('../tools/toolsService')
    const rule = updateHintRule(id, updates)
    return { success: !!rule, data: rule }
  })

  ipcMain.handle(IpcChannels.TOOLS_REMOVE_HINT_RULE, async (_, id: string): Promise<{ success: boolean; error?: string }> => {
    const { removeHintRule } = await import('../tools/toolsService')
    return { success: removeHintRule(id) }
  })

  ipcMain.handle(IpcChannels.TOOLS_MATCH_HINTS, async (_, input: string): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { matchHints } = await import('../tools/toolsService')
    return { success: true, data: matchHints(input) }
  })

  ipcMain.handle(IpcChannels.TOOLS_RESET, async (): Promise<{ success: boolean }> => {
    const { resetTools } = await import('../tools/toolsService')
    return resetTools()
  })

  ipcMain.handle(IpcChannels.TOOLS_GET_BY_ID, async (_, id: string): Promise<{ success: boolean; data?: any; error?: string }> => {
    const { getAllToolsData } = await import('../tools/toolsService')
    const tool = getAllToolsData().tools.find(t => t.id === id)
    return { success: !!tool, data: tool }
  })

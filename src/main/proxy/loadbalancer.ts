...
export class LoadBalancer {
  ...
  selectAccount(
    ...
    const excludeFailed = strategy === 'failover'
    const candidates = this.getAvailableAccounts(model, preferredProviderId, excludeFailed)

    if (candidates.length === 0) {
      // Build diagnostic info for better error messages
      const allProviders = storeManager.getProviders()
      const enabledProviders = allProviders.filter(p => p.enabled)
      const disabledProviders = allProviders.filter(p => !p.enabled)

      console.error(`[LoadBalancer] NO AVAILABLE ACCOUNT for model="${model}"`)
      console.error(`[LoadBalancer] Total providers: ${allProviders.length} (enabled: ${enabledProviders.length}, disabled: ${disabledProviders.length})`)

      if (enabledProviders.length === 0) {
        console.error(`[LoadBalancer] CAUSE: No enabled providers. Please enable a provider in settings.`)
      } else {
        for (const provider of enabledProviders) {
          const supports = this.providerSupportsModel(provider, model)
          const allAccounts = storeManager.getAccountsByProviderId(provider.id, true)
          const activeAccounts = allAccounts.filter(a => a.status === 'active')
          const accountsWithToken = allAccounts.filter(a => a.credentials?.token)
          console.error(`[LoadBalancer] Provider "${provider.name}" (${provider.id}): supportsModel=${supports}, totalAccounts=${allAccounts.length}, active=${activeAccounts.length}, hasToken=${accountsWithToken.length}`)

          if (!supports) {
            const effectiveModels = storeManager.getEffectiveModels(provider.id)
            console.error(`[LoadBalancer]   Supported models: ${effectiveModels.map(m => m.displayName).join(', ') || '(none)'}`)
          }
          if (activeAccounts.length === 0) {
            console.error(`[LoadBalancer]   CAUSE: No active accounts for this provider`)
            if (allAccounts.length === 0) {
              console.error(`[LoadBalancer]   SOLUTION: Add an account to provider "${provider.name}"`)
            } else {
              const statuses = allAccounts.map(a => `${a.name}(${a.status})`).join(', ')
              console.error(`[LoadBalancer]   Account statuses: ${statuses}`)
            }
          }
          if (accountsWithToken.length === 0 && activeAccounts.length > 0) {
            console.error(`[LoadBalancer]   CAUSE: Accounts exist but have no credentials/token`)
          }
        }
      }
  ...
  private getAvailableAccounts(
    ...
    for (const provider of providers) {
      ...
      for (const account of accounts) {
        const tokenLen = account.credentials.token ? account.credentials.token.length : 0
        console.log(`[LoadBalancer] Account ${account.name} (${account.id}) Token: ${tokenLen > 0 ? '[REDACTED]' : 'none'}`)
        candidates.push({
          account,
          provider,
  ...
  /**
   * Weighted strategy
   * Select account based on weight (higher weight = more likely to be selected)
   */
  private selectWeighted(candidates: AccountSelection[]): AccountSelection {
    const weightedCandidates = candidates.map(c => ({
      selection: c,
      weight: c.account.weight || c.provider.weight || 50,
    }))
    ...
    return weightedCandidates[weightedCandidates.length - 1].selection
  }

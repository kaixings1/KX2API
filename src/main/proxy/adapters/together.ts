/**
 * Together AI Adapter
 * Supports Together AI API (OpenAI-compatible)
 */
...
export class TogetherAdapter {
  ...
  async chat(request: TogetherRequest): Promise<TogetherResponse> {
    const url = this.getApiUrl()
    const headers = this.getHeaders()
    const transformed = { ...request, model: this.resolveModel(request.model) }

    logManager.info('[Together]', `Request to ${url}, model=${transformed.model}`)

    const response: AxiosResponse<TogetherResponse> = await axios.post(url, transformed, {
      headers,
  ...
  async *chatStream(request: TogetherRequest): AsyncGenerator<string, void, unknown> {
    const url = this.getApiUrl()
    const headers = this.getHeaders()
    const transformed = { ...request, model: this.resolveModel(request.model) }

    logManager.info('[Together]', `Stream request to ${url}, model=${transformed.model}`)
...
export const togetherAdapter = new TogetherAdapter(
  { id: 'together', name: 'Together AI', type: 'builtin', authType: 'token', apiEndpoint: 'https://api.together.xyz', headers: {}, enabled: true, createdAt: Date.now(), updatedAt: Date.now() } as Provider,
  { id: 'together-default', providerId: 'together', name: 'default', credentials: {}, status: 'active', createdAt: Date.now(), updatedAt: Date.now() } as Account
)

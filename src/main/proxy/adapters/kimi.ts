...
export class KimiAdapter {
  ...
  constructor(provider: Provider, account: Account) {
    ...
    this.token = account.credentials.token || account.credentials.refreshToken || account.credentials.accessToken || ''
  }
  ...
  private async acquireToken(): Promise<{ accessToken: string; userId: string }> {
    ...
    if (tokenType === 'jwt') {
      const expiry = getJwtExpiry(this.token)
      const userId = extractUserIdFromJWT(this.token) || ''

      // 若 JWT 已过期，尝试从持久会话（BrowserWindow cookie）获取 fresh token
      if (expiry !== null && expiry <= unixTimestamp()) {
        console.log('[Kimi] JWT token expired at', new Date(expiry * 1000).toISOString())
        accessTokenMap.delete(this.token)

        // 尝试从 kimiSessionManager 获取 fresh token，避免用户手动重登
        if (kimiSessionManager.ready()) {
          console.log('[Kimi] Token expired, trying persistent session...')
          try {
            const sessionToken = await kimiSessionManager.extractToken()
            if (!sessionToken) {
              console.log('[Kimi] No cached cookie, refreshing session window...')
              const refreshed = await kimiSessionManager.refreshSession()
              if (refreshed && detectTokenType(refreshed) === 'jwt') {
                console.log('[Kimi] Got fresh token from session refresh, length:', refreshed.length)
                this.token = refreshed
              }
            } else if (detectTokenType(sessionToken) === 'jwt') {
              const sessionExpiry = getJwtExpiry(sessionToken)
              if (sessionExpiry !== null && sessionExpiry > unixTimestamp()) {
                console.log('[Kimi] Using valid cookie from session, length:', sessionToken.length)
                this.token = sessionToken
              }
            }
          } catch (e) {
            console.log('[Kimi] Session refresh failed:', (e as Error).message)
          }
        }
      }

      // 使用当前 token（可能是 fresh token 或原始过期 token）建立缓存
      const currentExpiry = getJwtExpiry(this.token)
      const cacheDuration = currentExpiry !== null ? Math.min(currentExpiry - unixTimestamp(), 300) : 300
      accessTokenMap.set(this.token, {
        accessToken: this.token,
        refreshToken: this.token,
        userId,
        refreshTime: unixTimestamp() + Math.max(cacheDuration, 0),
      })
      console.log('[Kimi] Using JWT token, userId:', userId, 'expires:', currentExpiry !== null ? new Date(currentExpiry * 1000).toISOString() : 'unknown', 'cacheTTL:', Math.max(cacheDuration, 0) + 's')
      return { accessToken: this.token, userId }
    }

    console.log('[Kimi] Non-JWT token detected, using directly...')
    accessTokenMap.set(this.token, {
      accessToken: this.token,
      refreshToken: this.token,
      userId: '',
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse; conversationId: string }> {
    const { accessToken, userId } = await this.acquireToken()
    ...
    // Use existing chat for multi-turn, or empty string for new chat
    ...
    console.log('[Kimi] Request body length:', frameBuffer.length, 'JSON length:', frameBuffer.length - 5, 'chatId:', chatId || '(new)')
    ...
    if (response.status === 401) {
      accessTokenMap.delete(this.token)
      // 第一次重试：清除缓存重新获取 token
      console.log('[Kimi] 401 received, clearing cache and retrying (1/2)...')
      const { accessToken: retryToken } = await this.acquireToken()
      const retryPayload = createKimiChatPayload({
        model: request.model,
        content,
        enableWebSearch,
        enableThinking,
      ...
      const retryBuffer = encodeKimiGrpcFrame(retryPayload)
      ...
      if (retryResponse.status === 401) {
        // 第二次重试：尝试从持久会话（BrowserWindow cookie）获取 fresh token
        console.log('[Kimi] 401 still, trying persistent session cookie (2/2)...')
        console.log('[Kimi] Session manager ready:', kimiSessionManager.ready(), 'currentToken:', !!kimiSessionManager.getToken())
        accessTokenMap.delete(this.token)

        let freshToken: string | null = null
        try {
          if (kimiSessionManager.ready()) {
            console.log('[Kimi] Extracting token from persistent session...')
            freshToken = await kimiSessionManager.extractToken()
            console.log('[Kimi] extractToken result:', freshToken ? `length=${freshToken.length}` : 'null')
            if (!freshToken) {
              console.log('[Kimi] No cookie found, refreshing session window...')
              freshToken = await kimiSessionManager.refreshSession()
              console.log('[Kimi] refreshSession result:', freshToken ? `length=${freshToken.length}` : 'null')
            }
          } else {
            console.log('[Kimi] Session manager not ready, skipping persistent session retry')
          }
        } catch (e) {
          console.log('[Kimi] Session manager error:', (e as Error).message)
        }

        if (freshToken) {
          console.log('[Kimi] Got fresh token from session, retrying with new credential...')
          const freshPayload = createKimiChatPayload({
            model: request.model,
            content,
            enableWebSearch,
            enableThinking,
          ...
          const freshBuffer = encodeKimiGrpcFrame(freshPayload)

          const freshResponse = await axios.post(
            `${KIMI_API_BASE}/apiv2/kimi.gateway.chat.v1.ChatService/Chat`,
            freshBuffer,
            {
              headers: {
...
export class KimiStreamHandler {
  ...
  private handleMessage(
    ...
    if (data.done !== undefined) {
      ...
      // If no tool calls were emitted, try Kimi-native fallback on accumulated text
      const hasEmitted = this.toolStreamParser ? this.toolStreamParser.hasEmittedToolCall() : false
      if (!hasEmitted && this.fullTextBuffer.trim()) {
        const kimiCalls = this.parseKimiNativeToolCalls(this.fullTextBuffer)
        if (kimiCalls.length > 0) {
          for (let i = 0; i < kimiCalls.length; i++) {
            const tc = kimiCalls[i]
            transStream.write(`data: ${JSON.stringify({
              id: this.getConversationId(),
              model: this.model,
              object: 'chat.completion.chunk',
      ...
      transStream.write(`data: ${JSON.stringify({
        ...
        model: this.model,
        object: 'chat.completion.chunk',
        choices: [{ index: 0, delta: {}, finish_reason: hasEmitted ? 'tool_calls' : 'stop' }],
        created,
      })}\n\n`)
      this.fullTextBuffer = ''
      transStream.end('data: [DONE]\n\n')
  ...
  private sendChunk(transStream: PassThrough, content: string, created: number) {
    // Accumulate text for Kimi-native tool call detection on done
    this.fullTextBuffer += content

    // Process tool call interception via managedXml parser
    const chatId = this.getConversationId() || this.conversationId
    const baseChunk = createBaseChunk(chatId, this.model, created)
  ...
  private parseKimiNativeToolCalls(text: string): Array<{ id: string; function: { name: string; arguments: string } }> {
    ...
    while ((sectionMatch = sectionPattern.exec(text)) !== null) {
      ...
      const callPattern = /<\\|tool_call_begin\\|>\\s*([^<]+?)\\s*<\\|tool_call_argument_begin\\|>([\\s\\S]*?)<\\|tool_call_end\\|>/g
      let callMatch
      while ((callMatch = callPattern.exec(sectionContent)) !== null) {
        const fullName = callMatch[1].trim()
        const argsStr = callMatch[2].trim()
        let functionName = fullName
        if (fullName.startsWith('functions.')) {
          functionName = fullName.slice('functions.'.length)
        }
        const colonIdx = functionName.lastIndexOf(':')
        if (colonIdx > 0) {
          functionName = functionName.slice(0, colonIdx)
        }
        let args = '{}'
        try {
          args = JSON.stringify(JSON.parse(argsStr))
        } catch {
          args = argsStr || '{}'
        }
        calls.push({
  ...
  private buildToolCallChunk(toolCall: any, chatId: string, model: string, created: number, index: number): any {
    const baseChunk = createBaseChunk(chatId, model, created)
    let name = ''
    let toolArgs = '{}'
    if (toolCall.function) {
      name = toolCall.function.name || ''
      toolArgs = typeof toolCall.function.arguments === 'string' ? toolCall.function.arguments : JSON.stringify(toolCall.function.arguments || {})
    } else if (toolCall.name) {
      name = toolCall.name
      toolArgs = typeof toolCall.arguments === 'string' ? toolCall.arguments : JSON.stringify(toolCall.arguments || {})
    }
    const normalized = { id: toolCall.id || ('call_' + index), type: toolCall.type || 'function', function: { name, arguments: toolArgs } }
    return {
      ...baseChunk,
      choices: [{
        index: 0,

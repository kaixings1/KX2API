...
class SessionManagerClass {
  ...
  updateProviderSessionId(sessionId: string, providerSessionId: string): void {
    ...
    if (session) {
      storeManager.updateSession(sessionId, { providerSessionId } as Partial<SessionRecord>)
      console.log('[SessionManager] Updated providerSessionId for session:', sessionId)
    }
  }
  ...
  updateParentMessageId(sessionId: string, parentMessageId: string): void {
    const session = storeManager.getSessionById(sessionId)
    if (session) {
      storeManager.updateSession(sessionId, { parentMessageId } as Partial<SessionRecord>)
      console.log('[SessionManager] Updated parentMessageId for session:', sessionId)
    }
  }
  ...
  addMessage(sessionId: string, message: Omit<ChatMessage, 'timestamp'>): void {
    ...
    // Trim oldest messages if exceeding max
    if (messages.length > config.maxMessagesPerSession) {
      messages = messages.slice(messages.length - config.maxMessagesPerSession)
    }

...
router.post('/completions', async (ctx: Context) => {
  ...
  // 哨兵日志：确认路由被执行
  console.error(`[Chat][ROUTE-ENTER] requestId=${requestId} ip=${clientIP} time=${new Date().toISOString().slice(11,23)} model=${ctx.request.body?.model || '?'} stream=${ctx.request.body?.stream || '?'}`)

  console.log('[Chat][ROUTE] POST /v1/chat/completions requestId=', requestId, 'ip=', clientIP, 'time=', new Date().toISOString().slice(11,23))
  console.error(`[Chat][ROUTE-ENTER] requestId=${requestId} model=${request.model} stream=${request.stream} THIS-LOG-MUST-APPEAR`)

  let request: ChatCompletionRequest
  try {
    request = ctx.request.body as ChatCompletionRequest
    console.log('[Chat][ROUTE] body parsed model=', request.model, 'stream=', request.stream, 'messages=', request.messages?.length, 'tools=', request.tools?.length)
  } catch (error) {
  ...
  console.log('[Chat][ROUTE] validation passed, proceeding to feature headers')
  ...
  console.log('[Chat] selectAccount request.model=', request.model, 'preferredProviderId=', preferredProviderId, 'preferredAccountId=', preferredAccountId)
  ...
  console.log('[Chat] selected provider=', provider.id, 'account=', account.id, 'actualModel=', actualModel)

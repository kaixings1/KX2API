...
import { extractToolCallsFromText } from '../toolCallExtractor'
...
export class KiloCodePromptAdapter extends BasePromptAdapter {
  ...
  toolsToPrompt(tools: ChatCompletionTool[], variant?: PromptVariant): string {
    ...
    const toolDefinitions = tools.map(tool => {
      ...
      return `Tool \`${fn.name || 'unknown'}\`: ${fn.description || 'No description'}. Arguments JSON schema: ${params}`
    }).join('\n')
  ...
  transformRequest(
    ...
    if (this.hasPromptInjected(messages)) {
      console.log('[KiloCodeAdapter] Kilo Code prompt detected, cleaning and replacing with standard format')
      cleanedMessages = this.cleanKiloCodePrompt(messages)
      cleaned = true
    }

...
export function looksLikeToolCall(text: string): boolean {
  ...
  // 以 `{` / `[{` 开头**不再**直接判定为工具调用：
  //
  // 本函数的语义是「这是流式传输中的中间态，先别把它当正文输出」。
  // 但原实现对 `{` 一刀切，把「已完整闭合的 JSON 工具调用」也判成中间态，
  // 而调用方（extractToolCallsFromText）见到 true 就**直接返回空结果** ——
  // 于是正文与工具调用**双双丢失**，静默失败。
  //
  // 实测（tools/core-harness）：
  //   {"name": "ls", "arguments": {"path": "."}}
  //   → { content: "", toolCalls: [], protocol: "unknown" }
  //
  // 现在只有「配平不完整 / 解析不成功」才算中间态；完整 JSON 交回提取器处理。

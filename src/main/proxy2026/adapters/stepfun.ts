...
export class StepFunAdapter {
  ...
  private async acquireToken(): Promise<string> {
    ...
    logManager.debugLog(`[StepFun][AUTH] token prefix=${this.oasisToken.slice(0,20)}... webId=${this.webId ? this.webId.slice(0,20) : 'null'} hasValidPayload=${hasValidPayload} expiresAt=${expiresAt} isValid=${isValid}`)
  ...
  private buildHeaders(): Record<string, string> {
    ...
    // Send device_id as header if extractable from token (matches browser behavior)
    if (tokenDeviceId) {
      headers['Oasis-DeviceId'] = tokenDeviceId
    }
    logManager.debugLog(`[StepFun][AUTH] headers appId=${appId} webId=${this.webId ? this.webId.slice(0,20)+'...' : 'null'} tokenPrefix=${this.oasisToken.slice(0,20)}... cookieLen=${headers['Cookie']?.length || 0}`)

    // Build Cookie header exactly like the browser.
    // Cookie values are sent as-is, matching the browser behavior.
  ...
  private buildWebSessionRequest(request: ChatCompletionRequest, sessionId?: string): any {
    ...
    const cachedSession = this.getCachedSession(sessionKey)
    const chatSessionId = cachedSession?.chatSessionId || this.chatSessionId || (() => {
      // Generate UUID v4 matching browser's crypto.randomUUID() format
    ...
    // Build structured message content matching browser format
    const structuredContent = request.messages
      .map((msg) => {
    ...
    // Determine reasoning mode - frontend defaults to true for regular chat
    const hasReasoningEffort = request.reasoning_effort != null
    const enableReasoning = !hasReasoningEffort || request.reasoning_effort === 'high' || request.reasoning_effort === 'max'
    const enableSearch = false

    // Build message body matching browser format: {content: {userMessage: {qa: {content, attachments}}}}
    const qaContent = structuredContent.length === 1 ? structuredContent[0].content : structuredContent.map(m => m.content).join('\n')
    ...
    messageBody.chatSessionId = chatSessionId
    ...
    if (request.messages.length > 0) {
      ...
      if (lastMsg.chatId) {
        baseBody.config.fromChatcaseId = lastMsg.chatId
      }
    }
  ...
  async chatCompletion(request: ChatCompletionRequest, sessionId?: string): Promise<{ success: boolean; status?: number; stream?: Readable; body?: any; headers?: Record<string, string>; error?: string; chatId?: string }> {
    console.log('[StepFun] STEP-10 chatCompletion ENTRY isApiKeyMode=', this.isApiKeyMode, 'model=', request.model)
    if (!this.isApiKeyMode) {
      // Web session mode: use Connect protocol (HTTP streaming)
      await this.acquireToken()
      console.log('[StepFun] STEP-11 token acquired, calling chatCompletionConnect')
      return this.chatCompletionConnect(request, sessionId)
    }

    // API Key mode: use standard step_plan API (user's configured endpoint)
    console.log('[StepFun] STEP-11 API key mode, calling chatCompletionStepPlan')
    return this.chatCompletionStepPlan(request)
  }

  private async chatCompletionStepPlan(request: ChatCompletionRequest, retryCount = 0): Promise<{ success: boolean; status?: number; stream?: Readable; body?: any; headers?: Record<string, string>; error?: string; chatId?: string }> {
  ...
  private async chatCompletionConnect(request: ChatCompletionRequest, sessionId?: string): Promise<{ success: boolean; status?: number; stream?: Readable; body?: any; headers?: Record<string, string>; error?: string; chatId?: string }> {
    ...
    // chatSessionId MUST always be present — server signs the request against it.
    // Missing chatSessionId causes the server to return HTTP 200 with empty body.
    // Use crypto.randomUUID() to match the browser's format.
    if (!this.chatSessionId) {
      this.chatSessionId = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, c => {
        const r = (Math.random() * 16) | 0
        const v = c === 'x' ? r : (r & 0x3) | 0x8
        return v.toString(16)
      })
    }
    const useChatSessionId = cachedSession?.chatSessionId || this.chatSessionId
    let useChatId: string | null = cachedSession?.chatId || this.chatId

    // Register chatSessionId with server (browser calls POST /api/chat/session before Connect)
    if (!cachedSession?.chatSessionId) {
      try {
        const sessionRegBody = JSON.stringify({ chatSessionId: useChatSessionId })
        const sessionReg = net.request({
          method: 'POST',
          url: 'https://chat.stepfun.com/api/chat/session',
        })
        for (const [k, v] of Object.entries(headers)) {
          sessionReg.setHeader(k, v)
        }
        sessionReg.setHeader('Content-Type', 'application/json')
        sessionReg.write(sessionRegBody)
        const sessionRegResult = await new Promise<string>((resolve) => {
          let body = ''
          sessionReg.on('response', (resp) => {
            resp.on('data', (chunk) => { body += chunk.toString() })
            resp.on('end', () => resolve(body))
          })
          sessionReg.on('error', () => resolve(''))
          sessionReg.end()
        })
        console.log('[StepFun][SESSION] register result=', sessionRegResult.slice(0, 200))
      } catch (e) {
        console.warn('[StepFun][SESSION] register failed:', e)
      }
    }

    // DIAG: log final auth state before sending
    console.log('[StepFun][CONNECT] auth diag:',
      'chatSessionId=', useChatSessionId,
      'chatId=', useChatId,
      'tokenPrefix=', this.oasisToken.slice(0, 30),
      'webId=', this.webId ? this.webId.slice(0, 20) : 'null',
      'appId=', headers['Oasis-appID'],
    ...
    // Determine reasoning mode - frontend defaults to true for regular chat sends
    const hasReasoningEffort = request.reasoning_effort != null
    const enableReasoning = !hasReasoningEffort || request.reasoning_effort === 'high' || request.reasoning_effort === 'max'
    const enableSearch = false

    // Build request body matching the browser's Connect protocol format.
    // Connect frame JSON uses direct protobuf field mapping:
    //   { "content": { "userMessage": { "qa": { "content": "...", "attachments": [] } } } }
    const qaContent = structuredContent.length === 1 ? structuredContent[0].content : structuredContent.map(m => m.content).join('\n')
    ...
    // Include fromChatcaseId for chat continuation — only if we have a real chatId
    // (from a prior Connect response), NOT from the incoming OpenAI message metadata.
    if (useChatId) {
      requestData.config.fromChatcaseId = useChatId
    }
    ...
    console.log('[StepFun][CONNECT] headers:',
      ...
      'Oasis-Webid=', connectHeaders['Oasis-Webid'] ? connectHeaders['Oasis-Webid'].slice(0, 20) : 'null',
      'Oasis-Token=', connectHeaders['Oasis-Token'] ? '[REDACTED len=' + connectHeaders['Oasis-Token'].length + ']' : 'null',
      'Oasis-DeviceId=', connectHeaders['Oasis-DeviceId'] ? '[REDACTED]' : 'null',
    ...
    console.log('[StepFun][CONNECT] about to send request, model=', model, 'frameSize=', connectFrame.length)
    console.log('[StepFun][CONNECT] request body JSON=', JSON.stringify(requestData))
    console.log('[StepFun][CONNECT] request frame hex=', connectFrame.toString('hex'))
    console.log('[StepFun][CONNECT] request headers=', JSON.stringify(connectHeaders))
    logManager.debugLog(`[StepFun][REQ] model=${model} frameSize=${connectFrame.length}`)
    logManager.debugLog(`[StepFun][REQ] body=${JSON.stringify(requestData)}`)
    logManager.debugLog(`[StepFun][REQ] headers=${JSON.stringify(connectHeaders)}`)
    logManager.debugLog(`[StepFun][REQ] frameHex=${connectFrame.toString('hex')}`)
    ...
    return new Promise((resolve) => {
      ...
      request_.on('response', (response) => {
        ...
        console.log('[StepFun] STEP-12 CONNECT response status=', statusCode, 'headers=', JSON.stringify(responseHeaders).slice(0, 300))
        logManager.debugLog(`[StepFun][RESP] status=${statusCode} headers=${JSON.stringify(responseHeaders).slice(0,500)}`)
        ...
        response.on('data', (chunk: Buffer) => {
          ...
          // Resolve early when chatId is set so forwarder can cache it for multi-turn
          if (this.chatId) {
            tryResolve()
          }
        })
  ...
  private processConnectFrames(buffer: Buffer, stream: PassThrough, model: string, messageIdState: { current: string }): number {
    ...
    while (offset + 5 <= buffer.length) {
      ...
      try {
        ...
        console.log('[StepFun][CONNECT] parsed frame, event keys=', Object.keys(json).join(','), 'dataEventKeys=', json.data?.event ? Object.keys(json.data.event).join(',') : 'none')
        logManager.debugLog(`[StepFun][RESP] frame=${JSON.stringify(json).slice(0,1000)}`)
        ...
        logManager.debugLog(`[StepFun][RESP] parseError=${e} raw=${frameData.slice(0,200).toString('hex')}`)
      }
    }
  ...
  private handleConnectEvent(json: any, stream: PassThrough, model: string, messageIdState: { current: string }): void {
    ...
    if (json.error) {
      ...
      console.error('[StepFun][CONNECT] error frame:', errMsg, 'code=', errCode, 'full=', JSON.stringify(json).slice(0, 500))
      const errorChunk = {
        ...
        choices: [{
          index: 0,
          delta: { role: 'assistant', content: `[Error: ${errMsg}]` },
          finish_reason: 'error',
        }],
        created: Math.floor(Date.now() / 1000),
      }
    ...
    if (event.messageEvent) {
      ...
      if (!this.isApiKeyMode) {
        ...
        if (newChatSessionId || newChatId) {
          const cacheKey = sessionId || this.account.id
          this.setCachedSession(cacheKey, {
      ...
      console.log('[StepFun][CONNECT] messageEvent, messageId=', messageId, 'chatId=', msg.chatId, 'full=', JSON.stringify(event.messageEvent).slice(0, 300))
      return
    }
    ...
    if (event.textEvent) {
      const rawText = event.textEvent.text || ''
      console.log('[StepFun][CONNECT] textEvent, textLen=', rawText.length, 'textPreview=', rawText.slice(0, 100))
    ...
    if (event.doneEvent) {
      console.log('[StepFun][CONNECT] doneEvent, messageId=', messageIdState.current, 'full=', JSON.stringify(event.doneEvent).slice(0, 300))
      const finishChunk = {
        id: messageIdState.current,
        model: model,
        object: 'chat.completion.chunk',
    ...
    if (event.errorEvent) {
      ...
      const errorChunk = {
        ...
        choices: [{
          index: 0,
          delta: { role: 'assistant', content: `[Error: ${errorMsg}]` },
          finish_reason: 'error',
        }],
        created: Math.floor(Date.now() / 1000),
      }

...
class StoreManager {
   ...
   */
  private getEncryptionKeyPath(storagePath: string): string {
    return join(storagePath, '.encryption-key')
  }

  private readOrCreateEncryptionKey(): string | null {
    const storagePath = this.getStoragePath()
    const keyPath = this.getEncryptionKeyPath(storagePath)

    if (existsSync(keyPath)) {
      try {
        return readFileSync(keyPath, 'utf-8').trim()
      } catch (error) {
        console.warn('[Store] Failed to read encryption key file, generating new one:', error)
      }
    }
    ...
    try {
      ...
      console.error('[Store] Failed to write encryption key file:', error)
      return null
    }
  ...
  updateAccount(id: string, updates: Partial<Account>): Account | null {
    ...
    if (updates.credentials) {
      ...
      const redacted = Object.keys(updatedAccount.credentials).reduce((r, k) => {
        r[k] = typeof updatedAccount.credentials[k] === 'string' && updatedAccount.credentials[k].length > 8
          ? updatedAccount.credentials[k].slice(0, 4) + '...[REDACTED]'

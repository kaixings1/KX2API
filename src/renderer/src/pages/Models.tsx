...
export function Models() {
  ...
  const tabDescriptions: Record<string, string> = {
    list: t('models.modelListDesc', '查看和管理当前可用的模型列表'),
    mapping: t('models.modelMappingDesc', '配置模型名称映射，将模型 ID 映射为显示名称'),
    prompts: t('models.promptsDesc', '配置工具调用时的系统提示词模板'),
  }

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-[var(--accent-primary)]">{t('models.title')}</h2>

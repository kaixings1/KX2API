...
const PHASE_LABELS: Record<string, { label: string; variant: 'default' | 'secondary' | 'outline' | 'destructive' }> = {
  ...
  brainstorm: { label: '头脑风暴', variant: 'outline' },
  debate: { label: '讨论', variant: 'outline' },
  consensus: { label: '汇总', variant: 'secondary' },
  finalize: { label: '审查', variant: 'secondary' },
  'plan-generated': { label: '计划已生成', variant: 'default' },
...
export function TeamTaskPage() {
  ...
  const phaseBadge = PHASE_LABELS[phase] ?? { label: phase, variant: 'outline' as const }

  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">{t('team.title', '多角色任务')}</h2>

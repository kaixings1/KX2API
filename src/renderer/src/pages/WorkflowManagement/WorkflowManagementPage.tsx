  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <Card><CardContent className="py-8 text-center text-muted-foreground">没有匹配的工作流</CardContent></Card>
      ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs text-muted-foreground flex items-center gap-1"><ListOrdered className="h-3 w-3" /> {wf.steps.length} {t('workflows.stepsCount', '个步骤')}</p>
                      <div className="flex gap-1 flex-wrap">
                        {wf.steps.map((step, i) => (
                          <Badge key={step.id} variant="outline" className="text-[10px]">
                            {i + 1}. {step.type}: {JSON.stringify(step.config).slice(0, 30)}
                          </Badge>
                        ))}
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground">ID: {wf.id} | 创建: {new Date(wf.createdAt).toLocaleDateString()} | 更新: {new Date(wf.updatedAt).toLocaleDateString()}</p>
                    {execResult && (

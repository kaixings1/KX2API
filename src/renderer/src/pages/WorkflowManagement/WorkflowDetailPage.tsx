  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">ID: {workflow.id}</p>
          </div>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span>{workflow.steps.length} 个</span>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">{workflow.description || '(无描述)'}</p>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-2">
              耗时 {execResult.totalDurationMs}ms | {execResult.stepResults.length} 个步骤
            </p>
            <div className="space-y-1">
              {execResult.stepResults.map((r: any, i: number) => (
                <div key={i} className={`p-2 rounded text-xs ${r.success ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
                  Step {i + 1}: {r.success ? '成功' : '失败'} {r.error ? `- ${r.error}` : ''}
                </div>
              ))}
            </div>
          </CardContent>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>修改工作流配置</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>名称</Label>
            ...
            <div>
              <Label>步骤（格式: 类型:JSON配置，每行一个）</Label>

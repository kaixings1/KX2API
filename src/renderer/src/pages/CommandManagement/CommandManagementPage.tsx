...
import { Plus, Trash2, Play, Terminal, Loader2 } from 'lucide-react'
...
export function CommandManagement() {
  ...
  const loadCommands = useCallback(async () => {
    setLoading(true)
    console.log('[CMD-FE] loadCommands start')
    try {
      const res = await commandsApi.getAll()
      console.log('[CMD-FE] loadCommands result:', Array.isArray(res) ? res.length : 0, 'items')
      if (Array.isArray(res)) {
        const builtin = res.filter(c => c.type === 'builtin')
        const custom = res.filter(c => c.type === 'custom')
        console.log('[CMD-FE] builtin:', builtin.length, 'custom:', custom.length)
        console.log('[CMD-FE] builtin names:', builtin.map(c => c.name).join(', '))
      }
      setCommands(Array.isArray(res) ? res : [])
    } catch (e) {
      console.error('[CMD-FE] loadCommands error:', e)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">{t('commands.title', '命令管理')}</h2>

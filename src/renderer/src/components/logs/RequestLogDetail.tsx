          ...
          <div className="flex-1 min-h-0 px-6 py-4 overflow-y-auto">
            ...
            <TabsContent value="info" className="mt-0">
              <div className="grid grid-cols-2 gap-3">
                ...
                {log.chatUserInput && (
                  <InfoItem
                    label="Chat User Input"
                    value={<span className="text-xs break-all">{log.chatUserInput.slice(0, 100)}</span>}
                  />
                )}
                {log.chatEngineUrl && (
                  <InfoItem
                    label="Engine URL"
                    value={<span className="font-mono text-xs break-all">{log.chatEngineUrl}</span>}
                  />
                )}
                {log.chatEngineProvider && (
                  <InfoItem
                    label="Engine Provider"
                    value={log.chatEngineProvider}
                  />
                )}
                {log.chatProxyProviderName && (
                  <InfoItem
                    label="Proxy Provider"
                    value={log.chatProxyProviderName + (log.chatProxyAccountName ? ' / ' + log.chatProxyAccountName : '')}
                  />
                )}
                {log.chatProxyActualModel && log.chatProxyActualModel !== log.model && (
                  <InfoItem
                    label="Proxy Actual Model"
                    value={log.chatProxyActualModel}
                  />
                )}
                {log.chatUpstreamUrl && (
                  <InfoItem
                    label="Upstream URL"
                    value={<span className="font-mono text-xs break-all">{log.chatUpstreamUrl}</span>}
                  />
                )}
                {log.chatProxyProcessLatency && (
                  <InfoItem
                    label="Proxy Process Latency"
                    value={formatLatency(log.chatProxyProcessLatency)}
                  />
                )}
                {log.chatNotes && (
                  <InfoItem
                    label="Pipeline Notes"
                    value={<span className="text-xs break-all">{log.chatNotes}</span>}
                  />
                )}
              </div>
            </TabsContent>
            ...
            <TabsContent value="user" className="mt-0">
              {(log.chatUserInput || log.userInput) ? (
                <div className="space-y-4">
                  {log.chatUserInput && log.chatUserInput !== log.userInput && (
                    <div>
                      <SectionHeader
                        title="Chat User Input (raw)"
                        icon={<User className="h-4 w-4" />}
                        copyText={log.chatUserInput}
                      />

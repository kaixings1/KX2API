...
function mapOAuthCredentials(providerId: string | undefined, credentials: Record<string, string>): Record<string, string> {
  ...
  if (oauthKey && credentials[oauthKey]) {
    ...
    // Resolve token value once, outside the fieldName block so it's in scope for all providers
    ...
    if (fieldName) {
      result[fieldName] = tokenValue
    }
    // StepFun: also map device_id (from Oasis-Webid cookie alias) or web_id to web_id
    if (providerId === 'stepfun') {
      result.token = tokenValue
      result.web_id = credentials.device_id || credentials.web_id || ''
    }
    return result
  }
...
export function AddAccountDialog({
  ...
  const supportsOAuth = provider && ['deepseek', 'glm', 'kimi', 'mimo', 'minimax', 'qwen', 'qwen-ai', 'zai', 'perplexity', 'stepfun'].includes(provider.id)
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4 mt-4">
            ...
            {validationResult.error && (
              <div className="flex items-center gap-2 text-sm text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-500/15 p-3 rounded-lg">
            ...
            {validationResult.valid && validationResult.userInfo && (
              <div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/15 p-3 rounded-lg">

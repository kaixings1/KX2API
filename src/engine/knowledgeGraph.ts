 ...
 *
 * 移植来源：D:\src\engine\knowledgeGraph.ts（上游 Claude Code 引擎富矿，纯函数零依赖）
 */
...
export function buildKnowledgeGraph(files: GraphSourceFile[], options: BuildOptions = {}): KnowledgeGraph {
  ...
  const symbolByFile = new Map<string, Map<string, GraphNode>>() // file → name → node
...
export function getRelatedNodes(graph: KnowledgeGraph, nodeId: string): Array<{ node: GraphNode; relation: RelationType }> {
  ...
  for (const e of inEdges) {
    const to = graph.nodeById.get(e.from)
    if (to) out.push({ node: to, relation: e.relation })
  }
  return out
}

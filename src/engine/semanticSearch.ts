 ...
 *
 * 移植来源：D:\src\engine\semanticSearch.ts（上游 Claude Code 引擎富矿，
 * BM25 语义搜索纯函数 + node:fs 薄封装，Node/Electron 直接可用，无 bun 依赖）
 */

import { existsSync, mkdirSync, readdirSync, readFileSync as fsReadFile, statSync, writeFileSync, watch as fsWatch } from 'node:fs'
...
const STOP_WORDS = new Set(
  'the,and,for,are,but,not,you,all,can,her,was,one,our,out,has,have,that,this,with,from,they,would,there,their,what,about,which,when,make,like,time,just,know,take,people,into,year,your,good,some,could,them,see,other,than,then,now,look,only,come,its,over,think,also,back,after,use,two,how,work,first,well,way,even,new,want,because,any,these,give,day,most,been,had,did,get,got,much,many,where,each,why,still,being,every,between,need,down,should,both,same,last,long,little,own,here,old,tell,may,set,put,end,help,try,function,const,let,var,return,import,export,default,class,interface,type,public,private,async,await,new,this,throw,try,catch,finally,if,else,switch,case,break,continue,while,for,of,in,from,as,extends,implements,static,readonly,get,set,void,true,false,null,undefined,number,string,boolean,object,array,any,unknown,never,keyof,typeof,instanceof,find,search,look,explain,show,list,all,which,where,who,how,what,please,帮我,找出,搜索,查找,看看,解释,说明,显示,列出,所有,相关,关于,的,和,与,在,中,还,为,对,给,把,要,能,可以'.split(','),
)
...
export function analyzeQuery(query: string): QueryAnalysis {
  ...
  const hasCnAction = /(找出|搜索|查找|看看|解释|说明|显示|列出|分析|搜索)/.test(query)
  const action = actionM ? actionM[1].toLowerCase() : (hasCnAction ? 'search' : '')
  ...
  for (const n of nameTargets) {
    if (/^(the|and|for|are|but|not|you|all|can|was|one|our|out|has|have|that|this|with|from|they|would|there|their|what|about|which|when|make|like|time|just|know|take|people|into|year|your|good|some|could|them|see|other|than|then|now|look|only|come|its|over|think|also|back|after|use|two|how|work|first|well|way|even|new|want|because|any|these|give|day|most|been|had|did|get|got|find|search|list|show|explain|please)$/i.test(n)) continue
...
const SYMBOL_PATTERNS: Array<{ kind: IndexedSymbol['kind']; re: RegExp }> = [
  ...
  { kind: 'class', re: /(?:export\s+)?(?:void\s+)?class\s+([A-Za-z_$][\w$]*)/g },
  ...
  { kind: 'component', re: /(?:export\s+)?(?:void\s+)?([A-Z][A-Za-z0-9_$]*)\s*(?:=\s*(?:\([^)]*\)\s*=>|\([^)]*\)\s*=>)|\([^)]*\)\s*(?::[^=]+)?\s*=>|\([^)]*\)\s*\{)/g },
]
...
export class SemanticIndexer {
  ...
  load(): void {
    try {
      ...
      const raw = JSON.parse(fsReadFile(this.indexFile, 'utf-8'))
  ...
  save(): void {
    try {
      ...
      writeFileSync(this.indexFile, JSON.stringify(payload), 'utf-8')
    } catch { /* 保存失败忽略 */ }
  }
    ...
    try {
      ...
      for (const filePath of files) {
        try {
          ...
          const content = fsReadFile(filePath, 'utf-8')
  ...
  /** 单个文件增量更新（watch 回调使用） */
  private indexFileNow(filePath: string): void {
    try {
      const stat = statSync(filePath)
  ...
  watch(): void {
    try {
      ...
      this.watcher = fsWatch(this.projectRoot, { recursive: true }, (_event: string, filename: string | null) => {
        if (!filename) return
  ...
  getStats(): { files: number; chunks: number; symbols: number; indexSize: number } {
    ...
    let indexSize = 0
    try {
      if (existsSync(this.indexFile)) indexSize = statSync(this.indexFile).size
    } catch { /* ignore */ }

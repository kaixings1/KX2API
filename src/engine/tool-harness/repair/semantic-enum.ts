...
export function semanticMatchEnums(
	...
	const shape = schema as Record<string, { _def?: { typeName?: string; options?: string[] }; options?: string[] }>;
	...
	for (const [key, fieldSchema] of Object.entries(shape)) {
		...
		if (!isZodEnum(unwrapped)) continue;
...
function tokenize(s: string): string[] {
	...
	return spaced.toLowerCase().split(/[_\-\s]+/).filter((w) => w.length > 0);
}

...
test('managed xml parses valid KX2API tool call', () => {
...
test('managed xml parses KX2API canonical XML form', () => {
  const result = managedXmlProtocol.parse(
    '<|KX2API|tool_calls><|KX2API|invoke name="default_api:read_file"><|KX2API|parameter name="filePath">/tmp/a</|KX2API|parameter></|KX2API|invoke></|KX2API|tool_calls>',
    { tools, protocol: 'managed_xml' },
  )
...
test('managed XML parser rejects undeclared tool names and records invalid names', () => {
  const result = managedXmlProtocol.parse(
    '<|KX2API|tool_calls><|KX2API|invoke name="missing_tool">{}</|KX2API|invoke></|KX2API|tool_calls>',
    { tools, protocol: 'managed_xml' },
  )

...
function plan(protocol: ToolCallingPlan['protocol'] = 'managed_xml'): ToolCallingPlan {
  const fallbackStrategy: 'never' | 'always' = protocol === 'managed_xml' ? 'never' : 'always'
  return {
    mode: 'managed',
    protocol,
...
test('KX2API XML marker split across chunks emits a tool call', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  assert.deepEqual(parser.push('<|KX2API|tool_', baseChunk), [])
  const chunks = parser.push('calls><|KX2API|invoke name="default_api:read_file"><|KX2API|parameter name="filePath">/tmp/a</|KX2API|parameter></|KX2API|invoke></|KX2API|tool_calls>', baseChunk)

  assert.equal(chunks.at(-1)?.choices[0].delta.tool_calls[0].function.name, 'default_api:read_file')
})

test('partial KX2API start marker is reported as buffered so stream handlers do not leak it', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  const chunks = parser.push('<|KX2API|tool_calls', baseChunk)
...
test('text before tool call is preserved only before tool calling begins', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  const chunks = parser.push('before <|KX2API|invoke name="default_api:read_file"><|KX2API|parameter name="filePath">/tmp/a</|KX2API|parameter></|KX2API|invoke></|KX2API|tool_calls> after', baseChunk)
...
test('invalid tool name is not emitted as a tool call', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  const chunks = parser.push('<|KX2API|tool_calls><|KX2API|invoke name="missing"><|KX2API|parameter name="x">1</|KX2API|parameter></|KX2API|invoke></|KX2API|tool_calls>', baseChunk)
...
test('fenced code block examples are emitted as text and never as tool calls', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  const text = '```xml\n<|KX2API|tool_calls><|KX2API|invoke name="default_api:read_file"><|KX2API|parameter name="filePath">fake</|KX2API|parameter></|KX2API|invoke></|KX2API|tool_calls>\n```'
  const chunks = parser.push(text, baseChunk)

...
test('Mimo query preserves managed tool call history for follow-up requests', () => {
  ...
  assert.match(query, /<\|KX2API\|tool_calls>/)
  assert.match(query, /<\|KX2API\|invoke name="weather-test:get_weather">/)
  assert.match(query, /<\|KX2API\|tool_result tool_call_id="call_1">/)
...
test('Mimo stream converts managed XML into OpenAI tool calls', async () => {
  ...
  const stream = Readable.from([
    'event: message\n',
    'data: {"content":"<|KX2API|tool_calls><|KX2API|invoke name=\\"weather-test:get_weather\\"><|KX2API|parameter name=\\"city\\"><![CDATA[Hangzhou]]></|KX2API|parameter></|KX2API|invoke></|KX2API|tool_calls>"}\n\n',
  ])
  ...
  assert.doesNotMatch(output, /KX2API|KX2API/)
})

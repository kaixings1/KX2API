...
export async function* createAnthropicStreamFromOpenAI(input: {
  ...
  /**
   * 【补丁①】markdown 分段截断点定位
   * "加回车"等价于截断发送：断点之前的内容立即发送，之后的内容继续累积。
   * 规则（取最早出现的有效断点）：
   *  1) 冒号后一位（发送冒号及其之前），排除连续冒号
   *  2) `## ` 标题 → 断点在 `#` 前
   *  3) `- 英文` 列表项 → 断点在 `-` 前
   *  4) `---` 分隔线 → 断点在第一个 `-` 前
   * 断点位于缓冲开头（index<=0）时返回 null，交由原句子切分逻辑处理。
   */
  function findContentSplitIndex(text) {
    ...
    const m2 = text.match(/(^|\n)#{2,}\s/)
    if (m2 && m2.index != null) {
      const pos = m2.index + (m2[1] ? m2[1].length : 0)
      if (best === null || pos < best) best = pos
    }
    // 规则3：行首 `- ` + 英文字母 → 断点在 `-` 前
    const m3 = text.match(/(^|\n)-\s+[A-Za-z]/)
    if (m3 && m3.index != null) {
      const pos = m3.index + (m3[1] ? m3[1].length : 0)
      if (best === null || pos < best) best = pos
    }
    // 规则4：三连字符分隔线 `---` → 断点在第一个 `-` 前
    const m4 = text.match(/(^|\n)-{3,}\s*/)
    if (m4 && m4.index != null) {
      const pos = m4.index + (m4[1] ? m4[1].length : 0)
      if (best === null || pos < best) best = pos
    }
  ...
  async function* flushBufferedText() {
    ...
    while (textBuffer.length > 0) {
      if (inTable) {
        // 表格模式：不按句号切分，仅当遇到双换行或长度/时间超限时发送全部
        ...
        if (hasDoubleNewline || exceedsMaxLength || exceedsTime) {
          ...
          textBuffer = ''
          inTable = false // 发送后假设表格结束，若后续又出现表格特征会重新设置
        }
        break
      } else {
        // 【补丁①】markdown 截断点优先提前发送
        const splitIdx = findContentSplitIndex(textBuffer)
        if (splitIdx !== null) {
          const head = textBuffer.slice(0, splitIdx)
          ...
          // 断点之后的内容继续累积，交给下一次 flush 处理（可定位到下一个断点）
          textBuffer = textBuffer.slice(splitIdx)
          break
        }
